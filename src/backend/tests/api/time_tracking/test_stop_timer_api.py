...
def mock_use_case():
    mock = MagicMock()
    mock.stop_timer = AsyncMock(return_value={"status": "stopped"})
    return mock

@pytest.mark.anyio
async def test_stop_timer_success(client: AsyncClient, mock_use_case):
    app.dependency_overrides[get_time_tracking_use_case] = lambda: mock_use_case
    
    task_id = uuid.uuid4()
    response = await client.post(
        f"/api/v1/tasks/{task_id}/timer/stop",
        json={"reason": "manual"},
