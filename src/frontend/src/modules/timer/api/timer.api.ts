...
export async function getTimerLogs(
  ...
  const res = await http.get("/tasks/timer/logs", {
    params: {
      user_id: userId,
      task_id: taskId,
      board_id: boardId,
...
export async function getAllTimerLogs(userId: string): Promise<TimerLog[]> {
  const res = await http.get("/tasks/timer/logs", {
...
export async function deleteTimeLog(
  ...
  await http.delete(`/tasks/${taskId}/timer/logs/${logId}`, {
    params: { board_id: boardId },
  });
}
