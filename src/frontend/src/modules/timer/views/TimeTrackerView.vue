  ...
  <AppLayout>
    <div class="mb-6">
      ...
      <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">
        Riwayat waktu kerja dari card yang melibatkan Anda.
      </p>
    </div>
    ...
    <div
      v-if="activeTaskId"
      class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-emerald-200 dark:border-emerald-700 mb-6 px-5 py-4 flex items-center gap-4"
    >
      <span
        class="w-2.5 h-2.5 rounded-full bg-emerald-500 animate-pulse flex-shrink-0"
      />
      ...
      <span class="text-lg font-mono font-bold text-emerald-600 tabular-nums">{{
        formatTimer(elapsed)
      }}</span>
      <button
        @click="handleStopActiveTimer"
    ...
    <!-- Filters -->
    <div
      class="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 shadow-sm mb-6 p-4 flex flex-wrap items-end gap-3"
    >
      ...
      <div class="ml-auto text-right">
        ...
        <p
          class="text-lg font-bold text-gray-800 dark:text-gray-100 font-mono tabular-nums"
        >
          {{ formatTimer(filteredTotalSeconds) }}
        </p>
      </div>
    </div>
    ...
    <div
      v-if="loading"
      class="flex items-center justify-center h-40 text-gray-400 gap-2 text-sm"
    >
    ...
    <div
      ...
      class="bg-gray-50 dark:bg-gray-800 border border-dashed border-gray-300 dark:border-gray-600 rounded-2xl p-12 text-center"
    >
      <font-awesome-icon
        icon="clock"
        class="text-3xl text-gray-300 mb-3 block mx-auto"
      />
      <p class="text-sm text-gray-500 dark:text-gray-400 font-medium">
        Belum ada time entry
      </p>
      <p class="text-xs text-gray-400 dark:text-gray-500 mt-1">
        Mulai timer dari card di Boards untuk mulai tracking.
      </p>
    </div>
    ...
    <div v-else class="space-y-4">
      ...
      >
        <div
          class="flex items-center justify-between px-5 py-3 bg-gray-50 dark:bg-gray-700/40 border-b border-gray-100 dark:border-gray-700"
        >
          <span
          ...
          <div class="flex items-center gap-3">
            ...
            <span
              class="text-sm font-bold text-gray-700 dark:text-gray-200 font-mono"
              >{{ formatTimer(group.totalSeconds) }}</span
            >
          </div>
        </div>
        <div class="divide-y divide-gray-50 dark:divide-gray-700/60">
          ...
          >
            <div class="flex-1 min-w-0">
              <router-link
                :to="`/boards/${entry.boardId}`"
                class="text-sm text-gray-800 dark:text-gray-100 font-medium truncate block hover:text-blue-600 transition"
              >
                {{ entry.taskTitle }}
              </router-link>
              <div class="flex items-center gap-1.5 mt-0.5 flex-wrap">
                <span class="text-xs text-orange-500"
                  >{{ entry.boardTitle }} • {{ entry.columnTitle }}</span
                >
                <span
                  v-if="entry.activityDescription"
                  class="text-xs text-gray-400 dark:text-gray-500 truncate"
                  >— {{ entry.activityDescription }}</span
                >
                <span
                  v-if="formatStopReason(entry.stopReason)"
            ...
            <div
              class="flex items-center gap-1.5 text-xs text-gray-400 dark:text-gray-500 min-w-[120px] justify-center flex-shrink-0"
            >
              <font-awesome-icon icon="calendar" class="text-gray-300" />
              <span>{{
                entry.stopTime
                  ? `${formatClock(entry.startTime)} – ${formatClock(entry.stopTime)}`
                  : `${formatClock(entry.startTime)} – berjalan`
              }}</span>
            </div>
            <span
              class="text-sm font-mono font-semibold text-gray-700 dark:text-gray-200 min-w-[70px] text-right flex-shrink-0"
            >
              {{ formatTimer(entryDurationSeconds(entry)) }}
            </span>
            <div
              class="flex items-center gap-1 flex-shrink-0 w-[60px] justify-end"
            >
    ...
    <Teleport to="body">
      ...
      >
        ...
        <div
          class="relative bg-white dark:bg-gray-800 rounded-2xl shadow-xl border border-gray-200 dark:border-gray-700 w-full max-w-md p-5"
        >
          <h2 class="text-base font-bold text-gray-800 dark:text-gray-100 mb-1">
            Edit time entry
          </h2>
          ...
          <p class="text-[11px] text-gray-400 dark:text-gray-500 mt-2">
            Durasi dihitung ulang otomatis oleh server.
          </p>
          <p v-if="editError" class="text-xs text-red-500 mt-2">
            {{ editError }}
          </p>

          <div class="flex justify-end gap-2 mt-5">
            <button
              @click="closeEditEntry"
              class="text-sm px-3 py-1.5 rounded-lg text-gray-500 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700 transition"
            >
              Batal
            </button>
            <button
    ...
    <Teleport to="body">
      ...
      >
        ...
        <div
          class="relative bg-white dark:bg-gray-800 rounded-2xl shadow-xl border border-gray-200 dark:border-gray-700 w-full max-w-sm p-5"
        >
          <h2 class="text-base font-bold text-gray-800 dark:text-gray-100 mb-1">
            Hapus time entry?
          </h2>
    ...
    <Teleport to="body">
      <Transition name="toast">
        <div
          v-if="toast"
          class="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 bg-gray-900 text-white text-sm px-5 py-2.5 rounded-xl shadow-lg pointer-events-none"
        >
          {{ toast }}
        </div>
      </Transition>
...
const activeTaskId = ref<string | null>(
  localStorage.getItem("active_timer_task_id"),
);
const activeTaskTitle = ref<string | null>(
  localStorage.getItem("active_timer_task_title"),
);
const activeBoardId = ref<string | null>(
  localStorage.getItem("active_timer_board_id"),
);
const elapsed = ref(0);
...
const activeBoardTitle = computed(() => {
  return (
    boards.value.find((b: any) => b.id === activeBoardId.value)?.title ?? "-"
  );
});
...
async function handleStopActiveTimer() {
  ...
  try {
    ...
    elapsed.value = 0;
    await loadEntries(); // Sekarang sangat cepat & aman karena cuma 1x request ke getAllTimerLogs
  } catch (e: any) {
...
async function saveEditEntry() {
  ...
  try {
    ...
    // Update data di state lokal langsung (tanpa re-fetch API)
    const item = allEntries.value.find((e) => e.id === entry.id);
    if (item) {
      item.startTime = new Date(editStart.value).toISOString();
      item.stopTime = new Date(editStop.value).toISOString();
      item.activityDescription = editDescription.value.trim();
      item.durationSeconds = Math.round((stopMs - startMs) / 1000);
    }
  } catch (e: any) {
    editError.value =
      e?.response?.data?.error?.message || "Gagal memperbarui entry.";
  } finally {
...
async function confirmDeleteEntry() {
  ...
  try {
    ...
    // Hapus dari state lokal langsung (tanpa re-fetch API)
...
async function loadEntries() {
  ...
  loading.value = true;
  try {
    // Ambil semua timer logs milik user dalam 1x request ke API
    const logs = await getAllTimerLogs(myId);

    allEntries.value = logs.map((log) => ({
      id: log.id,
      taskId: log.task_id,
      taskTitle: log.task_title ?? "Untitled Task",
      boardId: log.board_id ?? "",
      boardTitle: log.board_title ?? "-",
      columnTitle: log.column_title ?? "-",
      labels: [],
    ...
    console.error("Gagal memuat time entries:", e);
    allEntries.value = [];
  } finally {
    loading.value = false;
  }
