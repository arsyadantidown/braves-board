...
export async function getAvailableBoardMembers(
  boardId: string,
): Promise<AvailableMember[]> {
  const res = await http.get(`/boards/${boardId}/members/available`);
  return res.data?.data ?? res.data ?? [];
}
...
export async function addBoardMember(
  ...
  const res = await http.post(`/boards/${boardId}/members`, {
    user_id: userId,
    role,
  });
  return res.data?.data ?? res.data;
...
export async function updateBoardMemberRole(
  ...
  const res = await http.patch(`/boards/${boardId}/members/${userId}`, {
    role,
  });
  return res.data?.data ?? res.data;
}
