...
export async function archiveTask(taskId: string, boardId: string) {
  const res = await http.patch(`/tasks/${taskId}/archive`, null, {
    params: { board_id: boardId },
  });
  return res.data?.data ?? res.data;
}
...
export async function unarchiveTask(taskId: string, boardId: string) {
  const res = await http.patch(`/tasks/${taskId}/unarchive`, null, {
    params: { board_id: boardId },
  });
  return res.data?.data ?? res.data;
}
...
export async function getTaskActivities(taskId: string, boardId: string) {
