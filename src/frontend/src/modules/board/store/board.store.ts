...
import {
  ...
  getAvailableBoardMembers as apiGetAvailableBoardMembers,
...
export const useAppStore = defineStore(
  ...
  () => {
    ...
    async function fetchBoards(force = false) {
      ...
      boards.value = Array.isArray(res)
        ? res
        : (res.boards ?? res.items ?? res.data ?? []);
    ...
    const availableMembers = ref<Record<string, AvailableMember[]>>({});
    ...
    async function fetchAvailableMembers(boardId: string) {
      const members = await apiGetAvailableBoardMembers(boardId);
      availableMembers.value[boardId] = members;
      return members;
    }

    async function addBoardMemberToStore(
      boardId: string,
      userId: string,
    ...
    async function updateBoardMemberRoleInStore(
      boardId: string,
      userId: string,
      role: BoardRole,
    ) {
    ...
    async function removeBoardMemberFromStore(boardId: string, userId: string) {
      ...
      if (boardMembers.value[boardId]) {
        boardMembers.value[boardId] = boardMembers.value[boardId].filter(
          (m) => m.user_id !== userId,
        );
      }
    }
    ...
    function getMyRole(
      ...
      return (
        boardMembers.value[boardId]?.find((m) => m.user_id === userId)?.role ??
        null
      );
    }
    ...
    function findColById(columnId: string) {
      for (const boardId in columnsByBoard.value) {
        const col = columnsByBoard.value[boardId].find(
          (c: any) => c.id === columnId,
        );
        if (col) return col;
      }
    ...
    function findBoardIdForColumn(columnId: string): string | null {
      for (const boardId in columnsByBoard.value) {
        if (columnsByBoard.value[boardId].some((c: any) => c.id === columnId))
          return boardId;
      }
      return null;
    }
    ...
    async function fetchTaskIdsByAssignee(
      boardId: string,
      assigneeId: string,
    ): Promise<Set<string>> {
      const cols = columnsByBoard.value[boardId] ?? [];
      const results = await mapLimit(cols, 4, (col: any) =>
        getTasks(col.id, boardId, assigneeId),
      );
    ...
    async function setTaskCompleteInStore(
      taskId: string,
      isCompleted: boolean,
    ) {
      const boardId = findBoardIdForTask(taskId);
      ...
      const found = findTaskInStore(taskId);
      if (found) {
        found.col.tasks[found.idx].is_completed =
          res?.is_completed ?? isCompleted;
      }
      return res;
    }
    ...
    async function archiveTaskInStore(taskId: string) {
      ...
      if (found) {
        ...
        if (!archivedByBoard.value[boardId])
          archivedByBoard.value[boardId] = [];
    ...
    async function unarchiveTaskInStore(taskId: string, boardId: string) {
      ...
      const cols = columnsByBoard.value[boardId] ?? [];
      const target =
        cols.find((c: any) => c.id === task._archivedFromColumnId) ?? cols[0];
      if (target) {
    ...
    return {
      ...
      addBoardMemberToStore,
      updateBoardMemberRoleInStore,
      removeBoardMemberFromStore,
      getMyRole,
      columnsByBoard,
      fetchColumns,
      addColumn,
      ...
      archivedByBoard,
      archiveTaskInStore,
      unarchiveTaskInStore,
      setTaskCompleteInStore,
      findBoardIdForColumn,
      findBoardIdForTask,
      fetchTasks,
      fetchTaskIdsByAssignee,
      addTask,
      editTask,
      removeTask,
      moveTaskToColumn,
