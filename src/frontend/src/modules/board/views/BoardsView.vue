<template>
  <Layout>
    ...
    <div class="flex items-center justify-between mb-4 px-1">
      <div class="flex items-center gap-3 min-w-0">
        ...
        <h1 class="text-lg font-bold text-gray-800 dark:text-gray-100 truncate">
      ...
      <div class="flex items-center gap-2">
        <div class="flex items-center -space-x-2">
          <div
            v-for="m in resolveMembers(boardMemberUserIds).slice(0, 5)"
            :key="m.id"
        ...
        <button
          ...
          :disabled="myTasksFilterLoading"
          :aria-pressed="onlyMyTasks"
          :title="
            onlyMyTasks ? 'Tampilkan semua task' : 'Tampilkan hanya task saya'
          "
        ...
        >
          ...
          <span
            ...
            class="text-[10px] bg-gray-100 dark:bg-gray-700 text-gray-500 dark:text-gray-300 rounded-full px-1.5"
    ...
    <div
      class="flex gap-3 overflow-x-auto pb-4 px-1"
      @click="openColumnMenuId = null"
    >
      ...
      <VueDraggable
        v-else
        v-model="columnsByBoard[boardId]"
        :animation="150"
      ...
      >
        <div
          v-for="board in columnsByBoard[boardId]"
          :key="board.id"
        ...
        >
          <!-- Column Header -->
          <div
            class="flex items-center justify-between px-3 py-2.5 border-b border-gray-200 dark:border-gray-700"
          >
            <div
              class="column-drag-handle flex items-center gap-2 flex-1 min-w-0 cursor-grab active:cursor-grabbing"
            >
              <input
                v-if="renamingColumnId === board.id"
                v-model="renameColumnTitle"
                @click.stop
                @keyup.enter="handleRenameColumnSubmit(board)"
                @keyup.esc="renamingColumnId = null"
                @blur="handleRenameColumnSubmit(board)"
                autofocus
              ...
              >
              <span
                class="text-xs text-gray-400 dark:text-gray-400 bg-white dark:bg-gray-700 border border-gray-200 dark:border-gray-600 rounded-full px-2 py-0.5 leading-none flex-shrink-0"
              >
            ...
            <div class="relative flex-shrink-0">
              <button
                @click.stop="toggleColumnMenu(board.id)"
                class="text-gray-400 hover:text-gray-600 hover:bg-gray-100 rounded px-1.5 py-0.5 transition text-base leading-none select-none"
              >
                ···
              </button>
              <div
                v-if="openColumnMenuId === board.id"
              ...
              >
                <button
                  v-if="canRenameColumn"
                  @click.stop="startRenameColumn(board)"
                  class="w-full text-left px-4 py-2 text-sm text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-gray-700 transition"
                >
                  Rename
                </button>
                <button
                  v-if="canDeleteColumn"
                  @click.stop="handleDeleteColumn(board)"
                  class="w-full text-left px-4 py-2 text-sm text-red-500 hover:bg-red-50 transition"
                >
                  Delete
                </button>
                <p
                  v-if="!canRenameColumn && !canDeleteColumn"
          ...
          <div v-if="canCreateTask" class="mx-2.5 mb-2.5 mt-1">
            <div v-if="addingTaskColumnId === board.id">
              <input
                ...
                @keyup.enter="handleCreateTask(board.id)"
                @keyup.esc="
              ...
              <div class="flex gap-2">
                <button
                  @click="handleCreateTask(board.id)"
            ...
            <button
              v-else
              @click="addingTaskColumnId = board.id"
        ...
        <div v-else class="bg-gray-50 rounded-xl border border-gray-200 p-3">
          ...
          <div class="flex gap-2">
            <button
              ...
              :disabled="boardCreating || !newBoardTitle.trim()"
              class="flex-1 bg-blue-500 text-white text-xs py-1.5 rounded-lg hover:bg-blue-600 disabled:opacity-50 transition"
            >
  ...
  <Teleport to="body">
    ...
    >
      <div
        class="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-4xl mx-4 flex flex-col overflow-hidden"
        style="max-height: 90vh"
        @click="closeAllDropdowns"
      >
        <!-- Top Bar -->
        <div
          class="flex items-center justify-between px-5 py-3 border-b border-gray-100 dark:border-gray-700 flex-shrink-0"
        >
          <div class="flex items-center gap-2">
            <div class="relative">
              <button
            ...
            <div class="relative">
              ...
              >
                <template v-for="item in ellipsisMenuItems" :key="item.action">
                  <div
                    v-if="item.action === 'leave' || item.action === 'delete'"
        ...
        <div class="flex flex-1 overflow-hidden">
          ...
          <div class="flex-1 overflow-y-auto px-6 py-5">
            <div class="flex items-start gap-3 mb-5">
              ...
              <h2
                ...
                @blur="
                  (e: FocusEvent) =>
                    handleTitleBlur((e.target as HTMLElement).innerText)
                "
            ...
            <div class="mb-6">
              <div class="flex flex-wrap gap-2 mb-2">
                <div class="relative">
                  ...
                  >
                    <div
                      class="flex items-center justify-between px-4 py-2.5 border-b border-gray-100 dark:border-gray-700"
                    >
                      <span class="text-xs font-semibold text-gray-500"
                        >Add to card</span
                      >
                      <button
                        @click.stop="addMenuOpen = false"
                ...
                <button
                  @click.stop="showAttachPanel = !showAttachPanel"
                  class="flex items-center gap-1.5 text-xs border border-gray-300 dark:border-gray-600 rounded-lg px-3 py-1.5 hover:bg-gray-50 dark:hover:bg-gray-700 text-gray-600 dark:text-gray-300 transition"
              ...
              <div class="flex flex-wrap items-center gap-4">
                ...
                <div>
                  ...
                  <div class="flex items-center gap-1.5 flex-wrap">
                    ...
                    >
                      ...
                      <button
                        @click.stop="handleToggleMember(m.id)"
                    ...
                    <div class="relative">
                      <button
                        ...
                        class="w-7 h-7 rounded-full border-2 border-dashed border-gray-300 flex items-center justify-center text-gray-400 hover:border-blue-400 text-sm"
                      >
                        +
                      </button>
                      <div
                      ...
                      >
                        <button
                          v-for="u in users"
                          :key="u.id"
                          @click.stop="handleToggleMember(u.id)"
                        ...
                        >
                          ...
                          <span
                            class="flex-1 truncate text-gray-700 dark:text-gray-200"
                            >{{ u.full_name || u.email }}</span
                          >
                          <font-awesome-icon
                            v-if="
                              (selectedTask?.assignee_ids ?? []).includes(u.id)
                            "
                            icon="check"
                ...
                <div>
                  ...
                  <div class="flex items-center gap-1.5">
                    <span
                      v-if="
                        dueDateStatus(selectedTask.dueDate) === 'overdue' ||
                        dueDateStatus(selectedTask.dueDate) === 'soon'
                      "
                      class="text-[10px] font-semibold px-1.5 py-0.5 rounded-full flex-shrink-0"
                      :class="
                        dueDateBadgeClass(dueDateStatus(selectedTask.dueDate))
                      "
                    >
                      {{
                        dueDateStatus(selectedTask.dueDate) === "overdue"
            ...
            <div class="mb-6">
              ...
              <textarea
                ...
                rows="3"
                class="w-full text-sm text-gray-600 dark:text-gray-200 border border-gray-200 dark:border-gray-600 rounded-lg px-3 py-2.5 outline-none focus:border-blue-400 resize-none transition placeholder-gray-400 bg-white dark:bg-gray-700"
              ></textarea>
            </div>

            <div
              class="mb-6 p-3 bg-gray-50 dark:bg-gray-700/40 border border-gray-200 dark:border-gray-700 rounded-xl"
            >
              <div class="flex items-center justify-between gap-3">
                ...
                <input
                  v-if="activeTimerTaskId !== selectedTask.id"
                ...
                <div class="flex items-center gap-2 flex-shrink-0">
                  ...
                  >
                    {{
                      activeTimerTaskId === selectedTask.id
                        ? formatTimer(timerSeconds[selectedTask.id] || 0)
                        : selectedTask.time || "00:00:00"
                    }}
                  </span>
                  <button
                    @click.stop="handleTimerToggle(selectedTask)"
                  ...
                  >
                    <font-awesome-icon
                      :icon="
                        activeTimerTaskId === selectedTask.id ? 'stop' : 'play'
                      "
                    />
                    {{
              ...
              >
                ...
                <p v-if="timerLogsLoading" class="text-xs text-gray-400">
                  Memuat…
                </p>
                <p v-else-if="timerLogsError" class="text-xs text-red-500">
                  {{ timerLogsError }}
                </p>
                <p v-else-if="!timerLogs.length" class="text-xs text-gray-400">
                  Belum ada time log.
                </p>

                <ul v-else class="space-y-1.5 max-h-40 overflow-y-auto">
                  <li
                    v-for="log in timerLogs"
                    :key="log.id"
                  ...
                  >
                    ...
                    >
                      <span class="text-gray-400 flex-shrink-0 w-20">{{
                        formatLogDate(log.start_time)
                      }}</span>
                      <span class="text-gray-700 tabular-nums flex-1">{{
                        formatTimerLog(log)
                      }}</span>
                      <span
                        v-if="formatStopReason(log.stop_reason)"
                    ...
                    <p
                      v-if="log.activity_description"
                      class="text-xs text-gray-400 mt-0.5 pl-20 truncate"
                    >
              ...
              <div class="space-y-1.5">
                ...
                >
                  <input
                    ...
                    @change="handleToggleSubtask(sub.id, !sub.completed)"
                    class="w-4 h-4 rounded accent-blue-600 cursor-pointer flex-shrink-0"
                  />
                  <span
                    contenteditable="true"
                    @keydown.enter.prevent="
                      (e: KeyboardEvent) => (e.target as HTMLElement).blur()
                    "
                    @blur="
                      (e: FocusEvent) =>
                        handleRenameSubtask(
            ...
            <!-- Attachments -->
            <div
              v-if="(selectedTask.attachments?.length ?? 0) > 0"
              class="mb-6"
            >
              <div class="flex items-center gap-2 mb-2">
                <font-awesome-icon
              ...
              <div class="space-y-2">
                <div
                  v-for="(att, i) in selectedTask.attachments"
                  :key="i"
                ...
                >
                  ...
                  <button
                    v-if="canDeleteTask"
                    @click="handleDeleteAttachment(att.id, i)"
          ...
          <div
            class="w-72 border-l border-gray-100 dark:border-gray-700 flex flex-col flex-shrink-0"
          >
            <!-- Tabs -->
            <div
              class="flex items-stretch border-b border-gray-100 dark:border-gray-700 px-2 pt-2 gap-1"
            >
              <button
            ...
            >
              ...
              >
                ...
                <div class="flex-1 min-w-0">
                  <div
                    class="flex items-baseline gap-1 flex-wrap justify-between"
                  >
                    <span
                    ...
                    >
                    <button
                      v-if="act.id && canDeleteTask"
                      @click="handleDeleteComment(act.id, i)"
                  ...
                  <div
                    class="mt-1.5 bg-gray-50 dark:bg-gray-700/50 border border-gray-200 dark:border-gray-600 rounded-lg px-3 py-2"
                  >
        ...
        <!-- Attachment Panel -->
        <div
          v-if="showAttachPanel"
          class="border-t border-gray-100 dark:border-gray-700 px-6 py-4 bg-gray-50 dark:bg-gray-700/40 flex-shrink-0"
        >
          <div class="flex items-center justify-between mb-3">
          ...
          <div class="flex gap-2 flex-wrap">
            ...
            <button
              @click="showAttachLink = !showAttachLink"
  ...
  <Teleport to="body">
    ...
    >
      <div
        class="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-md mx-4 flex flex-col overflow-hidden"
      ...
      >
        ...
        <div class="flex-1 overflow-y-auto px-5 py-4">
          ...
          <p v-else-if="!boardMemberList.length" class="text-xs text-gray-400">
            Belum ada member.
          </p>

          <div v-else class="space-y-2">
            <div
              v-for="m in boardMemberList"
              :key="m.id"
            ...
            >
              <div
                ...
                :class="memberColor(m.user_id)"
              >
                {{ resolveUserLabel(m.user_id).initial }}
              </div>
              <div class="flex-1 min-w-0">
                <p class="text-sm text-gray-700 dark:text-gray-200 truncate">
                  {{ resolveUserLabel(m.user_id).name }}
                </p>
              </div>
              <select
                v-if="canChangeMemberRole"
                :value="m.role"
                :disabled="isSoleOwner(m.user_id)"
                @change="
                  handleUpdateMemberRole(
                    m.user_id,
                    ($event.target as HTMLSelectElement).value as BoardRole,
                  )
                "
                :title="
                  isSoleOwner(m.user_id)
                    ? 'Satu-satunya owner — tidak bisa diubah'
                    : ''
                "
              ...
              <span
                v-else
                class="text-xs text-gray-500 bg-gray-100 rounded-full px-2 py-0.5 capitalize"
                >{{ m.role }}</span
              >
              <button
                ...
                :title="
                  isSoleOwner(m.user_id)
                    ? 'Tidak bisa menghapus satu-satunya owner'
                    : 'Remove'
                "
                class="transition text-xs px-1"
              >
                ✕
              </button>
            </div>
        ...
        <div
          ...
          class="border-t border-gray-100 dark:border-gray-700 px-5 py-4 flex-shrink-0"
        >
          <p
          ...
          <p v-if="!addableUsers.length" class="text-xs text-gray-400 mt-2">
            Semua user sudah jadi member.
          </p>
        </div>
      </div>
    </div>
  ...
  <Teleport to="body">
    <div
      ...
      class="fixed inset-0 z-50 flex items-center justify-center"
      style="background: rgba(0, 0, 0, 0.65)"
      @click.self="showArchivedPanel = false"
    >
      <div
        class="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-md mx-4 flex flex-col overflow-hidden"
        style="max-height: 85vh"
          ...
          <div v-else class="space-y-2">
            ...
            >
              <div class="flex-1 min-w-0">
                ...
                <p
                  v-if="t._archivedFromColumnTitle"
              ...
              <button
                @click="handleUnarchiveTask(t.id)"
  ...
  <Teleport to="body">
    <div
      v-if="deleteTaskConfirmOpen"
      class="fixed inset-0 z-[70] flex items-center justify-center"
      style="background: rgba(0, 0, 0, 0.65)"
      @click.self="deleteTaskConfirmOpen = false"
    >
      <div
        class="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-sm mx-4 p-5"
      >
        <p class="text-sm font-semibold text-gray-800 dark:text-gray-100 mb-2">
          Hapus card ini?
        </p>
        <p
          class="text-xs text-gray-500 dark:text-gray-400 leading-relaxed mb-5"
        >
          Card "{{ selectedTask?.title }}" akan dihapus permanen beserta
          subtask, komentar, dan attachment-nya. Tindakan ini tidak bisa
          dibatalkan.
        </p>
        <div class="flex gap-2 justify-end">
          <button
            @click="deleteTaskConfirmOpen = false"
...
} from "../utils/board-permission.util";
import {
  dueDateStatus,
  dueDateBadgeClass,
  dueDateInputValue,
...
const currentBoard = computed(() =>
  boardList.value.find((b: any) => b.id === boardId),
);
...
function visibleTaskCount(board: { tasks?: { id: string }[] }) {
  ...
  return onlyMyTasks.value
    ? tasks.filter((t) => myTaskIds.value.has(t.id)).length
    : tasks.length;
}
...
function resolveMembers(ids?: string[]) {
  ...
  return ids
    ...
    .map((u: any) => ({
      ...
      initial: ((u.full_name ?? u.email ?? "?") as string)
        .charAt(0)
        .toUpperCase(),
...
const assignedMembers = computed(() =>
  resolveMembers(selectedTask.value?.assignee_ids),
);
...
const boardMemberUserIds = computed(() =>
  boardMemberList.value.map((m: any) => m.user_id),
);
const myBoardRole = computed<BoardRole | null>(() =>
  store.getMyRole(boardId, currentUser.value?.id),
);
const canInviteMember = computed(() =>
  hasPermission(myBoardRole.value, "member.invite"),
);
const canRemoveMember = computed(() =>
  hasPermission(myBoardRole.value, "member.remove"),
);
...
const ownerCount = computed(
  () => boardMemberList.value.filter((m: any) => m.role === "owner").length,
);
...
const canCreateTask = computed(() =>
  hasPermission(myBoardRole.value, "task.create"),
);
const canDeleteTask = computed(() =>
  hasPermission(myBoardRole.value, "task.delete"),
);
const canArchiveTask = computed(() =>
  hasPermission(myBoardRole.value, "task.archive"),
);
const canCreateColumn = computed(() =>
  hasPermission(myBoardRole.value, "column.create"),
);
const canRenameColumn = computed(() =>
  hasPermission(myBoardRole.value, "column.rename"),
);
const canDeleteColumn = computed(() =>
  hasPermission(myBoardRole.value, "column.delete"),
);
...
async function handleDeleteColumn(column: any) {
  ...
  try {
    ...
    if (list)
      columnsByBoard.value[boardId] = list.filter(
...
async function handleTimerToggle(task: Task) {
  if (!task.id) {
    showToast("Task ID belum tersedia.");
    return;
  }
...
async function initBoards() {
  if (!boardId) {
    showToast("Board ID tidak ditemukan.");
    return;
  }
...
const currentColumnTitle = computed(() => {
  return (
    allColumns.value.find((c: any) => c.id === selectedTask.value?.column_id)
      ?.title ?? "-"
  );
});
...
async function handleMoveTask(toColumnId: string) {
  ...
  try {
    ...
    if (toColumn) await refreshActivityAfterAction();
    showToast("Task moved.");
  } catch (e: any) {
    showToast(apiErrorMessage(e, "Gagal memindahkan task."));
  }
...
const ellipsisMenuItems = computed(() => {
  ...
  if (canArchiveTask.value)
    items.push({ label: "Archive", action: "archive", danger: false });
  if (isOnThisCard.value)
    items.push({ label: "Leave card", action: "leave", danger: true });
...
async function handleToggleComplete() {
  ...
  try {
    ...
    if (e?.response?.status === 404 || e?.response?.status === 405) {
      showToast(
        'Fitur "mark complete" menunggu endpoint backend (PATCH /tasks/{id}/complete). Belum tersedia.',
      );
    } else {
...
const addMenuItems = [
  ...
  { label: "Members", action: "members", desc: "Assign members" },
  {
    label: "Attachments",
    action: "attachments",
    desc: "Add links, pages, work items, etc",
  },
];
...
async function loadPersistedActivities(task: Task) {
  ...
  try {
    const activities = await apiGetTaskActivities(task.id, boardId);

    if (selectedTask.value?.id !== task.id) return;

    // Pertahankan komentar yang sudah dimuat dari endpoint comments.
    const comments = task.activity.filter((activity) => activity.comment);

    const persistedActivities = activities.map(
      (activity: any): ActivityItem => {
        const user = resolveUserLabel(activity.user_id);
        const createdAt = new Date(activity.created_at).getTime() || 0;

        return {
          id: activity.id,
          author: user.name,
          initial: user.initial,
          color: memberColor(activity.user_id),
          action: activity.description || activity.action,
          date: new Date(activity.created_at).toLocaleString("id-ID", {
            day: "2-digit",
            month: "short",
            year: "numeric",
            hour: "2-digit",
    ...
    task.activity = [...comments, ...persistedActivities].sort(
...
async function handleAddLink() {
  ...
  if (!title || !url || !selectedTask.value?.id || attachLinkLoading.value)
    return;
