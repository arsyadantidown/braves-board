    ...
    <div class="p-6">
      ...
      >
        <div
          v-for="i in 4"
          :key="i"
          class="rounded-xl bg-gray-100 animate-pulse aspect-[4/3]"
        ></div>
      </div>
      ...
      >
        ...
        <p class="text-xs text-gray-400 mt-1">
          Buat board pertama Anda untuk mulai mengatur task.
        </p>
      </div>
      ...
      <div v-else class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
        <div
          ...
          class="group rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 overflow-hidden cursor-pointer hover:shadow-lg transition-all duration-150 aspect-[4/3] flex flex-col"
        >
          <div
            class="h-2/3 flex items-center justify-center relative flex-shrink-0"
            :class="boardGradient(board.id)"
          >
            <span class="text-3xl font-bold text-white/90 select-none">{{
            ...
            <div class="absolute top-2 right-2" @click.stop>
              ...
              <div
                ...
                class="absolute right-0 top-8 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl shadow-xl z-20 w-40 py-1 overflow-hidden text-left"
              >
                <button
                  @click.stop="startRename(board)"
                  class="w-full text-left px-4 py-2 text-sm text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-gray-700/60 transition"
                >
                  Rename
                </button>
                <button
                  @click.stop="startDelete(board)"
                  class="w-full text-left px-4 py-2 text-sm text-red-500 hover:bg-red-50 dark:hover:bg-red-500/20 transition"
                >
                  Delete
                </button>
              </div>
          ...
          <div
            class="flex-1 px-3.5 py-2.5 flex flex-col justify-center min-w-0"
          >
            <h3
      ...
      >
        <div
          class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl shadow-2xl p-6 w-full max-w-sm mx-4"
        >
          <h2 class="text-base font-bold text-gray-800 dark:text-gray-100 mb-4">
            Create Board
          </h2>
          <input
            v-model="newTitle"
            ...
            autofocus
            class="w-full text-sm border border-gray-200 dark:border-gray-600 rounded-lg px-3 py-2 outline-none focus:border-blue-400 transition mb-3 bg-white dark:bg-gray-700 dark:text-gray-100 placeholder-gray-400 dark:placeholder-gray-500"
          />
          <div class="flex gap-2">
            <button
              @click="handleCreate"
              :disabled="creating || !newTitle.trim()"
              class="flex-1 bg-blue-500 text-white text-sm py-2 rounded-lg hover:bg-blue-600 disabled:opacity-50 transition font-medium"
            >
            ...
            <button
              @click="
                ...
                newTitle = '';
              "
              class="text-sm text-gray-400 hover:text-gray-600 dark:hover:text-gray-200 px-3 transition"
            >
              Cancel
            </button>
          </div>
      ...
      <div
        ...
        @click.self="deletingBoard = null"
      >
        <div
          class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl shadow-2xl w-full max-w-sm mx-4 p-5"
        >
          <p
            class="text-sm font-semibold text-gray-800 dark:text-gray-100 mb-2"
          >
            Hapus board ini?
          </p>
          <p
          ...
          >
            Board "{{ deletingBoard.title }}" beserta seluruh column dan card di
            dalamnya akan tidak bisa diakses lagi. Tindakan ini tidak bisa
            dibatalkan.
          </p>
          <div class="flex gap-2 justify-end">
            <button
              @click="deletingBoard = null"
              class="text-xs text-gray-600 dark:text-gray-300 border border-gray-300 dark:border-gray-600 rounded-lg px-4 py-2 hover:bg-gray-50 dark:hover:bg-gray-700 transition"
            >
              Batal
            </button>
            <button
              @click="handleDeleteSubmit"
              class="text-xs text-white bg-red-500 hover:bg-red-600 rounded-lg px-4 py-2 transition font-medium"
            >
              Ya, hapus
            </button>
          </div>
...
async function handleRenameSubmit() {
  if (!renamingBoard.value || !renameTitle.value.trim() || renaming.value)
    return;
  renaming.value = true;
  try {
    ...
    showToast(
      e?.response?.data?.error?.message || "Gagal mengubah nama board.",
    );
  } finally {
